...
| Components inside the Android APK: Chaquopy / GeckoView / androidx / Kotlin / kotlinx.coroutines / Google Play services / the Python runtime and its dependencies | **Shipped with the Android package** (the APK embeds Chaquopy's Python runtime and GeckoView) | Each component's own license, **listed individually with versions and texts** | see `android/THIRD_PARTY_NOTICES.md` |
...
- **Android has its own notice**: the components inside the APK (Chaquopy, GeckoView, androidx, Kotlin, kotlinx.coroutines, Google Play services, the Python runtime and its dependencies) barely overlap with this file's list. They are listed separately in `android/THIRD_PARTY_NOTICES.md`, with their license texts in `android/licenses/`; ship that one with the APK and this one with the desktop artifacts.
