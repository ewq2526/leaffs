...
- **Logs now show the acting account**: each request-level access log line used to record only the IP — and on the same machine or the same WiFi the IP is often the same person, so it was impossible to tell **who** did something. Each line now carries `user=` and `role=` (anonymous is recorded as `-`, and the field count is fixed so columns stay aligned). In addition, clearing the archives of deleted users — an irreversible action — now writes an audit entry naming who did it and how many were removed.
