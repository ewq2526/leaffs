...
- **A refresh no longer makes it look like uploads finished**: if you refreshed or closed the page mid-upload, the files from that batch simply vanished with **no trace left on the page** — it looked exactly as if everything had uploaded. The in-flight batch is now recorded (in the current tab only) and the next time you open the page it says plainly: "Last batch did not finish: <names…> (please pick the files again)". ⚠️ It can **not** actually resume (that needs chunked upload), so the wording asks you to re-select the files rather than pretending it resumed.
- **User-management endpoints now report a bad parameter shape as a client error, not "internal server error"**: on `/api/users/` add, delete, password, role, quota, speed and rename, a non-string `username` / `role` / `old_name` (a number, object, array or `null`) used to raise and fall through to a 500 — which misleads the caller (it looks like a server fault, so they retry) and mixes such input in with real failures in the log. These now return an explicit 400.
- **Path normalization is now a single implementation**: `files/core.py` and `files/api.py` each carried a **character-for-character identical** copy of the path check (the two files deliberately do not import each other), and two identical checks inevitably drift — the recent "Windows device name" check nearly landed in only one of them. It now lives solely in `utils/core.normalize_rel_path`, and every other reference points at it.
- **Logs now show the acting account**: each request-level access log line used to record only the IP — and on the same machine or the same WiFi the IP is often the same person, so it was impossible to tell **who** did something. Each line now carries `user=` and `role=` (anonymous is recorded as `-`, and the field count is fixed so columns stay aligned). Deletions now also record **which files were removed** (the first few plus a total, so deleting thousands does not flood the log), and the acting account is recorded for clearing archives of deleted users, renaming a user, setting/clearing a share code, resetting a share brute-force counter, and toggling guest mode.
