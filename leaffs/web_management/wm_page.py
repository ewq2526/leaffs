...
from leaffs.config_core import cfg_core as _cfg
...
def serve_admin_page(handler, BASE_DIR, read_file_cached, is_default_admin_password):
    """渲染管理页面（注入角色和密码提醒）；语言 cookie 为 en 时优先英文版。"""
...
def serve_admin_users_page(handler, get_session, get_session_username, read_file_cached, BASE_DIR):
    """渲染用户管理页面（注入角色和用户名）；语言 cookie 为 en 时优先英文版。"""
    filepath = os.path.join(BASE_DIR, 'web_page', 'management', 'users.html')
    en_file = _en_variant_path(BASE_DIR, 'web_page/management/users.html',
                               handler.headers.get('Cookie', ''))
    if en_file:
        filepath = en_file
    data = read_file_cached(filepath)
...
def _en_variant_path(BASE_DIR, filename, cookie):
    """语言 cookie 为 en 且存在同名 .en.html 时，返回英文版文件路径；否则 None。"""
    try:
        if cookie and 'leaf_lang=en' in cookie and filename.endswith('.html'):
...
def serve_file(handler, filename, content_type, BASE_DIR,
    ...
    en_file = _en_variant_path(BASE_DIR, filename, cookie)
    if en_file:
        try:
