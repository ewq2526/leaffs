...
2. Sessions are kept in memory; all sign-ins become invalid after a server restart. Sign-in is rate-limited and lockable: at most 30 attempts per minute per egress IP; about 6 consecutive failures for the same account lock it for 10 minutes; 12 failures per 15 minutes at the account level cause a global lock. Devices behind the same NAT or proxy share one egress IP and therefore share the rate-limit counters.
...
5. Concurrency and resources: by default the whole machine allows up to 256 simultaneous connections, at most 20 HTTP connections per device and at most 8 WebSocket connections per device; excess connections are rejected immediately without queueing. Connections without data transfer are dropped after a timeout (120 seconds by default). All of the above can be adjusted on the Admin page and take effect immediately after saving.
