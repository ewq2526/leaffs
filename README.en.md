...
- Server-side local sign-in: the one-time local token link shown at startup (containing `?leaf=`) signs you in as Super Admin. The token is one-time and local-only; use it to enter the admin interface for the first time. ⚠️ **The Super Admin account has no password on first start** (there is no default password), so set one immediately after getting in. Do not send that link to anyone else.
...
- **Architecture and mechanism notes are in `docs/ARCHITECTURE.md`** (English: `docs/ARCHITECTURE.en.md`): layering and dependency direction, request flow, and the key mechanisms of the auth / permission / files / sharing / config / downloader domains, including why they are designed that way.
