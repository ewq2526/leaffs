...
def test_external_clear_is_picked_up(acc):
    """★ 另一处**清掉**分享码后，本进程必须立刻不再要求输码

    （旧实现会一直用旧值：码已经没了，本进程还在拦人 —— 只有重启才更新。）
    """
    S.set_code('alice', 'abcdef')
    assert S.code_enabled('alice') is True

    _external_edit(acc, lambda d: d['users']['alice'].update({'code_hash': ''}))

    assert S.code_enabled('alice') is False, (
        '外部清掉分享码后本进程还认为设了码 —— `_CACHE` 被当成唯一事实源了（§二 第 12 条）')


def test_external_set_is_picked_up(acc):
    """★ 反向：另一处**设上**分享码后，本进程必须立刻开始要求输码

    （旧实现里 A 处设码、B 处看不到 —— 列表与下载照旧放行，等于分享码不生效。）
    """
    assert S.code_enabled('bob') is False

    # 先让盘上确实有这个文件（真实场景里它是另一个进程写出来的）
    with open(acc, 'w', encoding='utf-8') as f:
        json.dump({'date': S._today(), 'users': {}, 'tickets': {}}, f)

    _external_edit(acc, lambda d: d.setdefault('users', {}).update(
        {'bob': {'code_hash': S.code_hash('abcdef')}}))

    assert S.code_enabled('bob') is True, (
...
def test_read_failure_does_not_mark_as_loaded(acc):
    ...
    """
    S.set_code('erin', 'abcdef')
    with open(acc, 'w', encoding='utf-8') as f:
        f.write('{ not json at all')
    st = os.stat(acc)
    os.utime(acc, ns=(st.st_atime_ns, st.st_mtime_ns + 1_000_000_000))

    S.code_enabled('erin')          # 触发一次读失败

    assert S._CACHE_STAMP != S._file_stamp(), (
        '读失败却把指纹更新了 —— 之后不会重试，要等文件再变')

    # 文件修好后应当能重新加载（指纹不匹配 ⇒ 会再试）
    with open(acc, 'w', encoding='utf-8') as f:
        json.dump({'date': S._today(), 'users': {'erin': {'code_hash': ''}}, 'tickets': {}}, f)
    st = os.stat(acc)
    os.utime(acc, ns=(st.st_atime_ns, st.st_mtime_ns + 1_000_000_000))
    assert S.code_enabled('erin') is False, '文件修好后没能重新加载'
