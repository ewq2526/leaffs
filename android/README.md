...
- **挂载手机上的目录要「所有文件访问权限」**：挂载（只读引用手机上的目录或文件）用系统
  选择器取路径，而服务端读文件走的是 `os.path`，读不了 SAF 的 `content://` —— 安卓 11 起
  只有「所有文件访问权限」（`MANAGE_EXTERNAL_STORAGE`）能给真实路径上的读权限。系统不给
  弹窗，App 会跳设置页，用户手动打开一次即可；没开时只是挂载这条路走不通，其余功能照常。
